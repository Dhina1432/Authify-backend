...
//            System.out.println("Password: " + request.getPassword());
            ...
            ResponseCookie cookie = ResponseCookie.from("jwt", jwtToken).httpOnly(true).secure(true).sameSite("none").path("/").maxAge(Duration.ofDays(1)).build();
    ...
    public void sendResetOtp(@RequestParam String email){
        try {
            System.out.println("SEND RESET OTP API HIT");
    ...
    public void verifyOtp(@CurrentSecurityContext(expression = "authentication?.name") String email,
        ...
        if (request.get("otp").toString()==null){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Missind Details");
